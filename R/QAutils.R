...
##
setMethod( "plot", "QAReadQuality", function( x, y, pal=brewer.pal( 8, "Set1" ), ... ){
...
##
## y is not used.
setMethod( "plot", "QANucleotideByCycle", function( x, y, dnaCol=c("#1F78B4", "#33A02C", "#B2DF8A", "#A6CEE3", "#E31A1C"), ... ){
    df <- as( values( x ), "data.frame" )
...
##
setMethod( "plot", "QAQualityByCycle", function( x, y, type="levelplot", ... ){
    return( plotQualByCycle( x=x, type=type, ... ) )
} )

plotQualByCycle <- function( x, type="levelplot" ){
...
###################################
## QANucleotideUse
setMethod( "plot", "QANucleotideUse", function( x, y, dnaCol=c("#1F78B4", "#33A02C", "#B2DF8A", "#A6CEE3", "#E31A1C"), ... ){
    return( plotNucleotideUse( x=x, ... ) )
} )
plotNucleotideUse <- function( x, dnaCol=c("#1F78B4", "#33A02C", "#B2DF8A", "#A6CEE3", "#E31A1C"), ... ){
    df <- as(values(x), "data.frame")
...
setMethod( "plot", "QAQualityUse", function( x, y, ... ){
    return( plotQualityUse( x=x, ... ) )
} )
plotQualityUseDensity <- function( x, xlab="Used base-call quality", ylab="density", col, pal=brewer.pal( 8, "Set1" ), lty=1, ... ){
    df <- as(values(x), "data.frame")
    lvl <- levels(df$Id)
    if( missing( col ) ){
...
setMethod( "plotDensity", "QAQualityUse", function( x, xlab="Used base-call quality", ylab="density", col, pal=brewer.pal( 8, "Set1" ), lty=1, ... ){
    plotQualityUseDensity( x, xlab=xlab, ylab=ylab, col=col, pal=pal, lty=lty, ... )
} )
